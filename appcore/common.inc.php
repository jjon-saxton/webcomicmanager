...
//		$pattern = "/([\w-:\*]*)(?:\#([\w-]+)|\.([\w-]+))?(?:\[@?(!?[\w-]+)(?:([!*^$]?=)[\"']?(.*?)[\"']?)?\])?([\/, ]+)/is";
	...
	*/
	...
	{
	  $classes=$this->classToArray();
          if ($key=array_search($name,$classes))
          {
            unset($classes[$key]);
          }
	  else
	  {
	    trigger_error("no classes named '{$name}' set for element",E_USER_NOTICE);
	  }
	  
	  return $this->attr['class']=implode(" ",$classes);
	}
	
	// remove this node *don't* forget to save the dom afterwards!
	function remove()
	{
